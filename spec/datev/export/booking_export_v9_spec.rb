...
describe Datev::BookingExportV9 do
  let(:booking1) {
    {
  ...
  let(:export) do
    export = Datev::BookingExportV9.new(
      'Herkunft'        => 'XY',
  ...
  describe :to_file do
    ...
    it 'should export a file identically to the given example' do
      Dir.mktmpdir do |dir|
        filename = "#{dir}/EXTF_Buchungsstapel_v9.csv"
        export.to_file(filename)
        if ENV['CREATE_EXAMPLES']
          export.to_file('examples/EXTF_Buchungsstapel_v9.csv')
        end

        expect(IO.read(filename)).to eq(IO.read('examples/EXTF_Buchungsstapel_v9.csv'))
      end
    end
  end
end
